...
class TestOptimizer(TestCase):
    ...
    def test_get_cob_matrix(self):
        ...
        cob_matrix = get_cob_matrix(np.array([0, 10, 20, 30, 40, 50, 60]), [10, 30])
        self.assertIsInstance(cob_matrix, np.matrix)
        self.assertEqual(14, cob_matrix.shape[0])
        self.assertEqual(7, cob_matrix.shape[1])
        cob_matrix = get_cob_matrix(np.array([0, 10, 20, 30]), [10, 30, 60, 90])
        self.assertIsInstance(cob_matrix, np.matrix)
        self.assertEqual(16, cob_matrix.shape[0])
        self.assertEqual(4, cob_matrix.shape[1])
